...
```{r}
#setwd("~/")
genotypes=read_table("exome_fto.raw",head=T) %>% select(FID,rs8050136_A)
```
...
```{r}
icd_codes=read_csv("id_codes.csv", col_types="ifci")
```
...
phenotypes=createPhenotypes(icd_codes, 
                            id.gender = covariates %>% transmute(id=FID, gender=ifelse(is.male,"M","F")),
                            aggregate.fun = sum)
```
