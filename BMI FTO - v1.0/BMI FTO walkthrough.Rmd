...
```{r}
my_plot=phewasManhattan(results, title="My example plot")
my_plot
```
...
Let's take a look at the effects of BMI on these associations. The phewas_ext function allows one run sets of associations with defined sets of covariates by providing a list of vectors instead of just one vector of covariates. No need to run the analysis again, but here's how one might do it.
```{r eval=FALSE}
